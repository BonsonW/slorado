...
- example slurm script: [Note 1](#note-1)

First, copy the example slurm script, and simply call `sbatch --account=${PAWSEY_PROJECT}-gpu example.sh` to submit the test job. 
