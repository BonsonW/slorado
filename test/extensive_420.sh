...
ex ./slorado basecaller models/$FAST test/4khz_r10/10_reads.blow5 -xcpu -c100 -K5 > test/tmp.fastq  || die "Running the tool failed"

echo "Memory Check - CPU - FAST model - incomplete batch"
ex ./slorado basecaller models/$FAST test/4khz_r10/10_reads.blow5 -xcpu -c100 -K6 > test/tmp.fastq  || die "Running the tool failed"
