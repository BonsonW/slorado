...
void calib_stats_t::accumulate(calib_layer_t *layer, const at::Tensor &input) {
    // Flatten all leading dims into one: (*, in_features) -> (n, in_features), on CPU.
    auto x = input.reshape({-1, input.size(-1)}).to(torch::kFloat32).cpu();
    ...
    // Per input-channel: max |x| over the batch/sequence dimension.
    auto ch_amax = std::get<0>(x.abs().max(/*dim=*/0));  // (in_features,), already on CPU
    if (!layer->x_per_in_ch_amax.defined()) {
        layer->x_per_in_ch_amax = ch_amax;
    } else {
        layer->x_per_in_ch_amax = torch::maximum(layer->x_per_in_ch_amax, ch_amax);
    }
...
void calib_stats_t::save_json(const std::string &path) const {
    ...
    for (size_t li = 0; li < sorted.size(); li++) {
        ...
        int64_t out_f = L->w_per_out_ch_amax.defined() ? L->w_per_out_ch_amax.size(0) : 0;
        int64_t in_f  = L->x_per_in_ch_amax.defined()  ? L->x_per_in_ch_amax.size(0)  : 0;
        ...
        fprintf(fp, "        \"per_in_channel_amax\": [");
        if (L->x_per_in_ch_amax.defined()) {
            const float *d = L->x_per_in_ch_amax.data_ptr<float>();
