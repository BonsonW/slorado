...
std::vector<torch::Tensor> load_tx_model_weights(const std::string &dir, const int num_convs, const int depth) {
    auto tensors = std::vector<std::string>{};
    tensors.reserve((size_t)num_convs * 2 + (size_t)depth * 7 + 3);

    // convs
    for (int i = 0; i < num_convs; ++i) {
        const auto conv = "conv." + std::to_string(i) + ".conv.";
        tensors.push_back(conv + "weight.tensor");
        tensors.push_back(conv + "bias.tensor");
    }

    // tx encoder layers
    for (int i = 0; i < depth; ++i) {
        const auto layer = "transformer_encoder." + std::to_string(i) + ".";
        tensors.push_back(layer + "self_attn.Wqkv.weight.tensor");
        tensors.push_back(layer + "self_attn.out_proj.weight.tensor");
        tensors.push_back(layer + "self_attn.out_proj.bias.tensor");
        tensors.push_back(layer + "ff.fc1.weight.tensor");
        tensors.push_back(layer + "ff.fc2.weight.tensor");
        tensors.push_back(layer + "norm1.weight.tensor");
        tensors.push_back(layer + "norm2.weight.tensor");
    }
...
ModuleHolder<AnyModule> load_tx_model(const CRFModelConfig &model_config, const torch::TensorOptions &options, tx_stats_t *model_stats, bool use_flash, int nthreads) {
    ...
    auto state_dict = load_tx_model_weights(model_config.model_path,
                                            (int)model_config.convs.size(),
                                            model_config.tx->tx.depth);
