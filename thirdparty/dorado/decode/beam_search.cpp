...
std::tuple<std::string, std::string> generate_sequence(const std::vector<uint8_t>& moves,
    ...
    for (size_t i = 0; i < seqLen; ++i) {
        ...
        qscore = std::min(qscore, 50.0f);
        qscore = std::max(qscore, 1.0f);
...
float beam_search(const T* const scores,
    ...
    for (size_t block_idx = 0; block_idx < num_blocks; ++block_idx) {
        ...
        block_prob = std::min(block_prob, 1.0f);
        block_prob = std::max(block_prob, 0.0f);
...
std::tuple<std::string, std::string, std::vector<uint8_t>> beam_search_decode(
    ...
    return make_tuple(std::move(sequence), std::move(qstring), std::move(moves));
}
