...
inline float log_sum_exp(float x, float y, float t) {
...
static at::Tensor scan(const torch::Tensor& Ms,
                const float fixed_stay_score,
...
static torch::Tensor forward_scores(const torch::Tensor& scores, const float fixed_stay_score) {
...
static torch::Tensor backward_scores(const torch::Tensor& scores, const float fixed_stay_score) {
    const int N = scores.size(1);  // Num batches
    const int C = scores.size(2);  // 4^state_len * 4 = 4^(state_len + 1)
...
std::vector<DecodedChunk> beam_search_cuda(const torch::Tensor& scores,
    ...
    auto t_scores = scores_cpu.index({Slice(), Slice(0, num_chunks)});
