 ...
 * default of 18000 means entries are calculated for differences of 0
 * to 18.000 nats (when p7_LOGSUM_SCALE is 1000.0).  e^{-p7_LOGSUM_TBL /
 * p7_LOGSUM_SCALE} should be on the order of the machine FLT_EPSILON,
 * typically 1.2e-7.
 */
#define p7_LOGSUM_TBL   18000
...
static inline int p7_FLogsumInit(void)
{
	...
	for (i = 0; i < p7_LOGSUM_TBL; i++) {
	    flogsum_lookup[i] = std::log1p(std::exp((double) (-i / temp) / p7_LOGSUM_SCALE)) * temp;
	}
	return 1;
}
...
static inline float p7_FLogsum(float a, float b) {
    extern float flogsum_lookup[p7_LOGSUM_TBL]; /* p7_LOGSUM_TBL=18000: (A-B) = 0..18 nats, steps of 0.001 */

    const float max = ESL_MAX(a, b);
    const float min = ESL_MIN(a, b);

    float abs_diff = std::abs(max-min);
    return max + ((abs_diff < 17.0f) ? flogsum_lookup[(int)((abs_diff) * p7_LOGSUM_SCALE)] : 0.0f);
}
