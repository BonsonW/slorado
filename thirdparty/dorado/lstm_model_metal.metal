// lstm_model_metal.metal — Metal compute kernels for the CRF LSTM basecaller on Apple Silicon.
//
// Vendored verbatim from dorado (nanoporetech/dorado, release-v2.0: dorado/nn/metal/nn.metal),
// which is used here under dorado's licence (see thirdparty/dorado/LICENCE). slorado uses the
// `lstm` and `reorder_*` kernels from this file via lstm_model_metal.mm to accelerate the
// standard-activation LSTM stack (fast/hac v5) on the MPS/Metal backend. Kept byte-for-byte so the
// tiled simdgroup-matmul layout matches dorado exactly; do not edit — update by re-vendoring.
//
...
// slorado edit: fp16 (was float) so the reorder-in kernel takes our fp16 conv output directly with
// no host-side fp32 conversion (the LSTM compute dtype `ftype` is fp16 anyway).
typedef half ftype_in;
...
// (Scores are int8_t.)
// slorado edit: fp16 (was float) so the reorder-out kernel writes fp16 directly for the CRF linear,
// avoiding an fp32->fp16 host conversion. Only affects the reorder_*_to_linear path slorado uses.
typedef half ftype_out;
