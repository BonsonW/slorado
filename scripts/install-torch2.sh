...
if [ ${DEV} = "cpu" ]; then
    ...
    if [ ${VER} = "2.0.0" ] ; then
        ...
        LINK="https://download.pytorch.org/libtorch/cu128/libtorch-shared-with-deps-2.9.1%2Bcu128.zip"
    else
    ...
    if [ ${VER} == "2.2.0" ] ; then
        ...
        LINK="https://download.pytorch.org/libtorch/rocm6.4/libtorch-shared-with-deps-2.9.0%2Brocm6.4.zip"
    else
