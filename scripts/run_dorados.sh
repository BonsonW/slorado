...
do
    clean_fscache
    if [ "$var" = "slorado" ]; then
        /usr/bin/time -v ./slorado basecaller -o ${OUTPUT_DIR}/slorado_calls.fastq -t 40 -x ${DEVICE} -B500M -C${GPU_BATCHSIZE} -K2000 -c10000 git${MODEL} ${BLOW5_FILE} 2>${OUTPUT_DIR}/slorado_log.txt
